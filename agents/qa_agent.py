...
def qa_node(state: QAState) -> QAState:
    ...
    try:
        ...
        if api_key and api_key != "dummy-key" and api_key.strip():
            ...
            Gere uma query SQL válida para DuckDB para responder à pergunta. Retorne APENAS um JSON no seguinte formato:
            {{
                "sql_executed": "SUA QUERY SQL AQUI",
                "narrative": "Uma explicação em português sucinta e profissional sobre os resultados obtidos (descreva o faturamento, volumes, etc.)"
            }}
            ...
            print("QA Agent: Google API Key not configured. Using Offline Fallback Compiler...")
            ...
            if "quantos hospitais" in q or "hospitais distintos" in q or "lista de hospitais" in q:
                sql = "SELECT hospital_nome as Hospital, cnes as CNES, hospital_municipio_nome as Cidade, hospital_uf as UF FROM master_healthcare GROUP BY ALL ORDER BY hospital_nome"
                narrative_builder = lambda df: f"Foram identificados {len(df)} estabelecimentos de saúde distintos no banco de dados do Acre."
            ...
            # Rule 2: Cataract volume by hospital
                ...
                def narrative_builder(df):
                    if df.empty:
                        return "Nenhuma cirurgia de catarata registrada no período de competência analisado."
                    top_h = df.iloc[0]['Hospital']
                    top_v = df.iloc[0]['Volume de Cirurgias']
                    tot_v = df['Volume de Cirurgias'].sum()
                    return f"Foram realizadas {tot_v} cirurgias de catarata no total. O estabelecimento líder em volume é {top_h}, com {top_v} cirurgias realizadas."
            ...
            # Rule 3: Revenue / financial totals
                ...
                def narrative_builder(df):
                    if df.empty:
                        return "Sem dados de faturamento."
                    total_pago = df['Faturamento Total (R$)'].sum()
                    top_h = df.iloc[0]['Hospital']
                    top_f = df.iloc[0]['Faturamento Total (R$)']
                    return f"O faturamento total em saúde identificado foi de R$ {total_pago:,.2f}. O hospital com maior receita captada foi o {top_h}, com R$ {top_f:,.2f}."
            ...
            # Rule 4: Underserved municipalities
                ...
                def narrative_builder(df):
                    ...
                    return f"Análise de cobertura: identificamos {zero_count} municípios no Acre sem registros de atendimento/cirurgias de catarata para seus residentes no mês (volume zero), indicando possíveis gargalos de acesso."
            ...
            # Rule 5: Hospital stay / average days
                ...
                def narrative_builder(df):
                    ...
                    return f"A média geral de permanência dos pacientes internados varia por hospital. O maior tempo médio de internação foi registrado no {top_h}, com {top_d} dias por paciente."
            ...
            # Default fallback query
                ...
                """
                narrative_builder = lambda df: f"Consulta geral executada com sucesso. Encontrados registros de {len(df)} hospitais operando no período."
